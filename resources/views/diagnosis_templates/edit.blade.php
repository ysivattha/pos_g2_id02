...
@section('title')
    {{__('lb.edit_diagnosis_templates')}}
@endsection
@section('header')
    {{__('lb.edit_diagnosis_templates')}}
@endsection
...
<div class="toolbox pt-1 pb-1">
    ...
    <form action="{{url('diagnosis-template/update')}}" method="POST">
        @csrf
    <button class="btn btn-primary btn-sm">
        <i class="fa fa-save"></i> {{__('lb.save')}}
    </button>
    <a href="{{route('diagnosis_template.index')}}">
    <button class="btn btn-success btn-sm" type="button">
        <i class="fa fa-reply"></i> {{__('lb.back')}}
    </button>
</a>
@cancreate('diagnosis_template')
<a href="{{route('diagnosis_template.create')}}">
    <button class="btn btn-success btn-sm" type="button" >
    ...
    <div class="card-body">
        ...
        <div class="row">
            <div class="col-sm-6">
                ...
                <div class="form-group row">
                    <div class="col-sm-12">
                        <textarea name="description" id="description" rows="3" class="form-control ckeditor">{{$tem->description}}</textarea>
                    </div>
                </div>
            </div>
