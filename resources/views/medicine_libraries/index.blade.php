...
<div class="modal fade" id="createModal" tabindex="-1" role="dialog" aria-labelledby="createModal" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <form action="uytyut" method="POST" id='create_form' onsubmit="frm_submit(event)">
          @csrf
          <input type="hidden" name="tbl" value="medicine_libraries">
          <input type="hidden" name="per" value="medicine_library">
            ...
            <div class="modal-header bg-success">
              ...
              <div class="modal-body">
                ...
                <div class="form-group row">
                        ...
                        <label class="col-md-3" for="category_id">
...
<div class="modal fade" id="editModal" tabindex="-1" role="dialog" aria-labelledby="editModal" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <form action="uytyut" method="POST" id='edit_form' onsubmit="frm_update(event)">
          ...
          <div class="modal-content">
            <div class="modal-header bg-success">
                <strong class="modal-title">{{__('lb.edit_medicine_library')}}</strong>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
                </button>
            </div>
              <div class="modal-body">
                ...
                <div class="form-group row">
                        ...
                        <label class="col-md-3" for="ecategory_id">
                            {{__('lb.category')}} <span class="text-danger">*</span>
                        </label>
                
                    <div class="col-md-9">
                    <select name="category_id" class="chosen-select" id='ecategory_id'>
                        <option value="">{{__('lb.select_one')}}</option>
                        @foreach ($categories as $cat)
                        <option value="{{$cat->id}}">{{$cat->name}}</option>
                        @endforeach
                ...
                <div class="form-group row">
                        ...
                        <label class="col-md-3" for="enote">
                            {{__('lb.note')}} 
                        </label>
                 
                    <div class="col-md-9">
                        <input type="text" name="note" id="enote" class="form-control input-xs">
                    </div>
                </div>
...
<script src="{{asset('chosen/chosen.jquery.min.js')}}"></script>
	<script>
        $(document).ready(function () {
            ...
            $("#menu_medicine_library").addClass("myactive");
			...
			var table = $('#dataTable').DataTable({
                ...
                serverSide: true,
                scrollX: true,
                ajax: "{{ route('medicine_library.index') }}",
                columns: [
                    {
                        data: 'check', 
                        name: 'check', 
        ...
        function edit(id, obj)
        {
            ...
            $.ajax({
                ...
                success: function(sms)
                {
                    ...
                    $('#ecategory_id').val(data.category_id);
                    $('#ecategory_id').trigger('chosen:updated');
