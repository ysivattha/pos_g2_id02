...
<div class="toolbox pt-1 pb-1">
    @cancreate('template')
    <a href="{{route('template.create')}}" class="btn btn-success btn-sm">
    ...
    <a href="{{route('template.edit', $tem->id)}}" class="btn btn-success btn-sm">
        <i class="fa fa-edit"></i> {{__('lb.edit')}}
    </a>
 
    @endcanedit
    @candelete('template')
    <a href="{{route('template.delete', $tem->id)}}" class="btn btn-danger btn-sm" 
        onclick="return confirm('{{__('lb.confirm')}}')">
        <i class="fa fa-trash"></i> {{__('lb.delete')}}
    </a>
    @endcandelete
    @endif
   <a href="{{route('template.index')}}" class="btn btn-success btn-sm">
