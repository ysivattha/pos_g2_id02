...
<div class="modal fade" id="createModal" tabindex="-1" role="dialog" aria-labelledby="createModal" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <form action="#" method="POST" id='create_form' onsubmit="frm_submit(event)">
          @csrf
          <input type="hidden" name="tbl" value="protocol_categories">
          <input type="hidden" name="per" value="protocol_category">
          <div class="modal-content">
            <div class="modal-header bg-success">
                <strong class="modal-title">{{__('lb.create_protocol_category')}}</strong>
...
<div class="modal fade" id="editModal" tabindex="-1" role="dialog" aria-labelledby="editModal" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <form action="uytyut" method="POST" id='edit_form' onsubmit="frm_update(event)">
          ...
          <div class="modal-content">
            <div class="modal-header bg-success">
                <strong class="modal-title">{{__('lb.edit_protocol_category')}}</strong>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
                </button>
            </div>
...
@section('js')
	<script>
        $(document).ready(function () {
            ...
            $("#menu_protocol_category").addClass("myactive");
			...
			var table = $('#dataTable').DataTable({
                ...
                serverSide: true,
                scrollX: true,
                ajax: "{{ route('protocol_category.index') }}",
                columns: [
                    {
                        data: 'check', 
                        name: 'check', 
